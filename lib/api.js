...
const api = axios.create({
    baseURL: "http://127.0.0.1:5001/careerfair-development/us-central1/api/v1/",
});
...
const getAssignedSessions = async (student_id) => {
    try {
        const assignedSessions = await api.get(
            `/student/sessions/${student_id}`
        );
...
const updatePanelAvailability = async (id, body) => {
    try {
        const response = await api.post(
            `/company/panel/updateavalabilty/${id}`,
            body
        );
        return response;
    } catch (error) {
